...
# Read protein list
prot_df = pd.read_csv('primary_data/avoidome_prot_list.csv')

# Only keep rows with ChEMBL_target (fix linter error)
prot_df = prot_df.loc[pd.notna(prot_df['ChEMBL_target']) & (prot_df['ChEMBL_target'] != '')]
...
for idx, row in prot_df.iterrows():
    ...
    rows = []
    for act in activities:
        # Use 'target_organism' for organism if available, else fallback to 'organism', else 'unknown'
    ...
    if not out_df.empty:
        out_df.to_csv('processed_data/avoidome_bioactivity_profile.csv', index=False, mode='w' if first_write else 'a', header=first_write)
        # Optionally, if you want to write to avoidome_bioactivity_profile_1.csv as well:
        # out_df.to_csv('processed_data/avoidome_bioactivity_profile_1.csv', index=False, mode='w' if first_write else 'a', header=first_write)
        first_write = False
