#!/usr/bin/env python3
"""
ESM + Morgan QSAR Modeling with Physicochemical Descriptors
...
- ESM C embeddings (sequence-level)
...
# ESM C imports
sys.path.append('/home/serramelendezcsm/RA/Avoidome/analyses/run_esm_embeddings')
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/home/serramelendezcsm/RA/Avoidome/analyses/standardized_qsar_models/esm_morgan_qsar.log'),
...
class ESMMorganQSARModel:
    """
    QSAR model using ESM C embeddings, Morgan fingerprints, and physicochemical descriptors
    """
    ...
    def __init__(self, 
        ...
        """
        Initialize the QSAR model with ESM C embeddings
        ...
        self.regression_dir = self.output_dir / "esm_morgan_regression"
        self.classification_dir = self.output_dir / "esm_morgan_classification"
    ...
    def calculate_esm_embedding(self, protein_sequence: str) -> Optional[np.ndarray]:
        """
        Calculate ESM C embedding for a protein sequence
        
        Args:
            protein_sequence: Protein sequence string
            
        Returns:
            ESM C embedding array or None if failed
        """
        try:
            if not protein_sequence or pd.isna(protein_sequence):
                return None
                
            logger.info(f"Calculating ESM C embedding for sequence of length {len(protein_sequence)}")
            
            # Calculate ESM C embedding
            ...
            # Ensure we get a 1D array (sequence-level embedding)
            if embedding.ndim > 1:
            ...
            logger.info(f"ESM C embedding calculated successfully (dimension: {len(embedding)})")
            return embedding
            
        except Exception as e:
            logger.error(f"Error calculating ESM C embedding: {e}")
            return None
    ...
    def prepare_features(self, activities_df: pd.DataFrame, protein_sequence: str) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        """
        Prepare features (ESM + Morgan + physicochemical) and targets for modeling
        ...
        # Calculate ESM C embedding
        logger.info("Calculating ESM C embedding...")
        esm_embedding = self.calculate_esm_embedding(protein_sequence)
        if esm_embedding is None:
            raise ValueError("Failed to calculate ESM C embedding")
        
        # Replicate ESM embedding for all compounds
        esm_features = np.tile(esm_embedding, (len(smiles_list), 1))
        ...
        # Combine features: ESM + Morgan + Physicochemical
        features = np.hstack([esm_features, morgan_fps, physchem_descriptors])
        
        logger.info(f"Feature matrix shape: {features.shape}")
        logger.info(f"ESM C embedding: {esm_features.shape[1]} features")
    ...
    def train_regression_model(self, features: np.ndarray, targets: np.ndarray, 
        ...
        """
        logger.info(f"Training ESM+Morgan regression model for {protein_name} ({uniprot_id})")
        ...
        results = {
            ...
            'organism': organism,
            'model_type': 'esm_morgan_regression',
        ...
        logger.info(f"ESM+Morgan regression model completed for {protein_name}")
    ...
    def train_classification_model(self, features: np.ndarray, targets: np.ndarray, 
        ...
        Returns:
            Dictionary with model results
        """
        logger.info(f"Training ESM+Morgan classification model for {protein_name} ({uniprot_id})")
        ...
        results = {
            ...
            'uniprot_id': uniprot_id,
            'organism': organism,
            'model_type': 'esm_morgan_classification',
            'threshold': threshold,
        ...
        logger.info(f"ESM+Morgan classification model completed for {protein_name}")
    ...
    def save_model_results(self, results: Dict, model_type: str, organism: str):
        ...
        Args:
            ...
            model_type: 'esm_morgan_regression' or 'esm_morgan_classification'
        ...
        if model_type == 'esm_morgan_classification':
        ...
        # Get the actual ESM C embedding dimension from the feature importance array
        esm_dim = len(results['feature_importance']) - 2048 - 14  # Total - Morgan - Physicochemical
        feature_names = [f"esm_{i}" for i in range(esm_dim)] + \
    ...
    def process_organism(self, protein_name: str, organism: str, uniprot_id: str) -> Dict:
        ...
        protein_sequence = self.get_protein_sequence(uniprot_id)
        if protein_sequence is None:
            logger.warning(f"No sequence found for {protein_name} ({organism})")
            return {
                'protein_name': protein_name,
                'organism': organism,
                'uniprot_id': uniprot_id,
                'status': 'skipped',
                'reason': 'no_sequence',
                'n_samples': len(activities_df)
            }
        
        try:
            # Prepare features
            features, targets, smiles_list = self.prepare_features(activities_df, protein_sequence)
            
            # Train regression model
            logger.info(f"Training ESM+Morgan regression model for {protein_name} ({organism})")
            regression_results = self.train_regression_model(features, targets, protein_name, uniprot_id, organism)
            self.save_model_results(regression_results, 'esm_morgan_regression', organism)
            
            # Train classification model
            logger.info(f"Training ESM+Morgan classification model for {protein_name} ({organism})")
            classification_results = self.train_classification_model(features, targets, protein_name, uniprot_id, organism)
            self.save_model_results(classification_results, 'esm_morgan_classification', organism)
    ...
    def run_all_models(self):
        ...
        logger.info("Starting ESM+Morgan QSAR modeling for all proteins")
        ...
        summary_path = self.output_dir / "esm_morgan_modeling_summary.csv"
        ...
        logger.info("=" * 60)
        logger.info("ESM+MORGAN MODELING SUMMARY")
        logger.info("=" * 60)
        ...
        logger.info("ESM+Morgan modeling completed!")


def main():
    """Main function to run ESM+Morgan QSAR modeling"""
